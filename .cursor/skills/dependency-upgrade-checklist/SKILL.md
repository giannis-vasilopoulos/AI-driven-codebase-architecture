...
- Upgrading a core piece of the stack: Next.js, React, TypeScript, Apollo Client, React Query,
  ESLint
...
4. Run the full definition-of-done: `npm run typecheck`, `npm run lint`, `npm run test`,
   `npm run build`. A version bump that "just installs" without running these isn't done.
...
6. For framework upgrades (Next.js, React): check bundle size before/after
   (`npm run build` output) and skim the migration guide's "breaking changes" section against
